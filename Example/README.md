...
2026-09-23：发布版本的 `Sources` 与主题修复提交 `4910cb9` 一致。该提交在应用管理状态栏的测试宿主下通过 45 项 XCTest（0 失败、0 跳过），覆盖全局主题设置、单次覆盖、相机隔离、预览往返以及状态栏恢复。发布准备仅更新版本号和文档，未重复执行相同运行代码的测试；控制器管理状态栏的既有 43 项测试结果见上文。宿主实际手势、相机、相册写入及权限流程仍需分别验收。
